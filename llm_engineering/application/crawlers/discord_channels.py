...
class DiscordChannelsCrawler(BaseLocalCrawler):
    ...
    @bot.command()
    async def describe_by_id(ctx, channel_id: int):
        """
        Gets the topic of a channel by its ID.
        Usage: !describe_by_id <channel_id>
        Example: !describe_by_id 123456789012345678
        """
        try:
            channel = ctx.guild.get_channel(channel_id)  # Try to get channel from the current guild
            if not channel:
                channel = self.bot.get_channel(channel_id)  # Fallback to global cache if not in current guild
                if not channel:
                    return await ctx.send(f"Could not find a channel with ID: `{channel_id}`.")

            if isinstance(channel, discord.TextChannel):
                topic = channel.topic
                if topic:
                    await ctx.send(f"The topic for <#{channel.id}> is: ```{topic}```")
                else:
                    await ctx.send(f"<#{channel.id}> does not have a topic set.")
            else:
                await ctx.send(f"The ID `{channel_id}` belongs to a non-text channel.")
        except Exception as e:
            await ctx.send(f"An error occurred: {e}")

    def extract(self, link: str, **kwargs) -> None:
        logger.info(f"Starting scrapping data for Discord channel: {link}")
